# Title
## Hospital Appointment Management System
A simple and secure system for managing hospital appointments with user authentication.

## Features
- User Registration & Login
- JWT Authentication
- Role Based Access (Admin / Patient)
- Create, Update, Delete Appointments
- Password Hashing with Bcrypt
- Protected Routes

## API URL = https://hospital-backend-ghsn.onrender.com

## Authentication API Routes
| Method  | Route                        | Description                     | Role  |
|---------|------------------------------|---------------------------------|-------|
| POST    | /api/auth/admin/register      | Register a new admin            | Admin |
| POST    | /api/auth/user/register       | Register a new user             | User  |
| POST    | /api/auth/admin/login         | Login and get JWT token         | Admin |
| POST    | /api/auth/user/login          | Login and get JWT token         | User  |
| GET     | /api/auth/admin/profile/:id   | Get profile                     | Admin |
| GET     | /api/auth/user/profile/:id    | Get profile                     | User  |
| PUT     | /api/auth/admin/update-user/:id   | Update details                  | Admin |
| PUT     | /api/auth/user/update-user/:id    | Update details                  | User  |
| PATCH   | /api/auth/admin/change-password/:id   | Update password                 | Admin |
| PATCH   | /api/auth/user/change-password/:id    | Update password                 | User  |
| DELETE  | /api/auth/admin/delete-accounte/:id   | Delete account                  | Admin |
| DELETE  | /api/auth/user/delete-account/:id    | Delete account                  | User  |
| POST    | /api/auth/admin/refresh-token | Refresh JWT token               | Admin|
| POST    | /api/auth//user/refresh-token  | Refresh JWT token               | User |
| PUT    | /api/auth/admin/upload-image/:id  | Upload profile image            | Admin|
| PUT    | /api/auth/user/upload-image/:id   | Upload profile image            | User |
| DELETE    | /api/auth/admin/delete-image/:id  | delete profile image            | Admin|
| DELETE    | /api/auth/user/delete-image/:id   | delete profile image            | User |

# 1. Repository clone
git clone https://github.com/MRP-rakib/hospital-backend.git

# 2. Project folder
cd hospital-backend

# 3. Dependencies install
npm install

# 4. Development server
npm run dev
